...
module.exports = function getPatternImportData (paths, options) {
  ...
  if(patternTemplate){
    ...
    // determine style file or array of files
    var patternStyle = utils.getPatternStyleName(patternObject, options);
    // add pattern style file to files-to-copy array
    if(patternStyle){
      // convert to array and iterate through entries
      [].concat(patternStyle).forEach(function (stylesheet) {
        var patternFileToCopy = {
          type: 'styles',
          src: path.join(stylesheet),
          dest: path.join(patternFiles.patternStylesPath, stylesheet)
        }
        patternFiles.filesToCopy.push(patternFileToCopy);
      })
    }
